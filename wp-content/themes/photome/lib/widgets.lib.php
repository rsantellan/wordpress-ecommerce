...
class Custom_Instagram extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		{
			$photos_arr = tg_get_instagram($pp_instagram_username, $pp_instagram_access_token, $items);
			...
			if(!empty($photos_arr))
			{
				...
				{
					echo '<li>';
					echo '<a target="_blank" href="'.esc_url($photo['link']).'"><img src="'.esc_url($photo['thumb_url']).'" alt="" width="75" height="75" /></a>';
					echo '</li>';
				}
