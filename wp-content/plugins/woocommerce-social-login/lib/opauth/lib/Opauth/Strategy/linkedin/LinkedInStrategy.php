...
class LinkedInStrategy extends OpauthStrategy{
	...
	public function oauth2callback(){
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			...
			if (!empty($results) && !empty($results->access_token)){
				...
				$this->mapProfile($profile, 'picture-urls.picture-url', 'info.image');
	...
	private function getProfile($access_token){
		if (empty($this->strategy['profile_fields'])) {
			$this->strategy['profile_fields'] = array('id', 'first-name', 'last-name', 'maiden-name', 'formatted-name', 'headline', 'industry', 'summary', 'email-address', 'picture-urls::(original)', 'location:(name)', 'public-profile-url', 'site-standard-profile-request');
		}
