...
class FacebookStrategy extends OpauthStrategy{
		...
		if (array_key_exists('code', $_GET) && !empty($_GET['code'])){
			...
			if (!empty($results) && !empty($results['access_token'])){
				...
				$this->auth = array(
					...
					'info' => array(
						'name' => $me->name,
						'image' => 'https://graph.facebook.com/'.$me->id.'/picture?width=400&height=400'
					),
					'credentials' => array(
	...
	private function me($access_token){
		$me = $this->serverGet('https://graph.facebook.com/v2.4/me', array('access_token' => $access_token, 'fields' => 'email,name,first_name,last_name,link'), null, $headers);
		if (!empty($me)){
